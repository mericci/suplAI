/**
 * Organization Rule Database Queries
 *
 * Data access layer for organization_rules table.
...
/**
 * Fetch all active rules for an org (global + supplier-specific).
 */
...
/**
 * Fetch all active rules for an org scoped to a specific supplier (or global).
 */
...
/**
 * Find the applicable rule for a given org, supplier, and amount.
 * Supplier-specific rules take precedence over global rules.
 * Returns null if no rule matches (caller should use hardcoded defaults).
 */
export async function findApplicableRule(
  ...
  // Fallback: global rules (supplier_id IS NULL)
...
/**
 * Soft-delete all rules for an org + supplier scope, then insert new ones.
...
export async function replaceRules(
  ...
  // Soft-delete existing rules for this scope
