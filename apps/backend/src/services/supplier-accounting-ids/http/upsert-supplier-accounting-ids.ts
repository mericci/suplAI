/**
 * HTTP handler: POST /api/organizations/:orgId/suppliers/:supplierId/accounting-ids
 */

import { upsertSupplierAccountingIds } from '../handlers/index.js';
import { successResponse, serverError, validationError } from '../../../utils/response.js';
import { getErrorMessage } from '../../../utils/error.js';
import { isValidUUID } from '../../../utils/validation.js';
import type { UpsertSupplierAccountingIdsPayload } from '../types/index.js';
...
export async function upsertSupplierAccountingIdsHandler(req: Request): Promise<Response> {
  try {
    ...
    for (const a of payload.assignments) {
      if (!a.accountingId || !isValidUUID(a.accountingId)) {
        return validationError('Each assignment must have a valid accountingId (UUID)');
      }
