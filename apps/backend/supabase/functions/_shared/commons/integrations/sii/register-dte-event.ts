/**
 * SII DTE Event Registration (Edge Function)
 *
...
function buildSoapEnvelope(params: Omit<RegisterDteEventParams, 'siiToken'>): string {
...
export async function registerDteEvent(
  ...
  const response = await fetch(REGISTRO_EVENTOS_URL, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`SII SOAP error ${response.status}: ${await response.text()}`);
  }
}
